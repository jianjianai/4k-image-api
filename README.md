...
Modelslab Real-ESRGAN 云端超分处理器。它会先判断用户请求的 `size` 是否超过配置的最大尺寸；如果超过，就把生成请求的 `size` 等比缩小到最大尺寸内，生成完成后把 provider 输出图片提交到 Modelslab super resolution API 放大，再下载返回的图片 URL。如果没有超过最大尺寸，则不会调用 Modelslab。

```json
{
  "id": "resize-modelslab-4x",
...
| `modelId` | 否 | `RealESRGAN_x4plus`、`RealESRGAN_x4plus_anime_6B` 或 `RealESRGAN_x2plus`，默认 `RealESRGAN_x4plus` |
| `scale` | 否 | 放大倍率，默认 `4` |
