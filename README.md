...
本地尺寸适配器。它会先判断用户请求的 `size` 是否超过配置的最大尺寸；如果超过，就把生成请求的 `size` 等比缩小到最大尺寸内，生成完成后再用 `sharp` 的 Lanczos3 kernel 把图片放大回用户原始请求尺寸。如果没有超过最大尺寸，则不修改请求和输出。
...
| `fit` | 否 | 输出放大回原尺寸时的 sharp resize fit，`contain`、`cover`、`fill`、`inside` 或 `outside`，默认 `fill` |

### `size-adapter:aliyun:super-resolution`

阿里云视觉智能开放平台「图像超分」云端处理器。它会先判断用户请求的 `size` 是否超过配置的最大尺寸；如果超过，会从 `1x`、`2x`、`3x`、`4x` 中选择最小可用倍率，反推出发送给生成 provider 的尺寸，生成完成后通过阿里云官方 SDK 调用 `MakeSuperResolutionImage`，再下载返回的图片 URL。如果没有超过最大尺寸，则不会调用阿里云。

阿里云版本不会在云端放大后再用本地 `sharp` 做最终 resize。为了让最终尺寸由超分接口直接产出，请求的宽高必须能被所选倍率整除，并且反推后的生成尺寸要在 `maxWidth`、`maxHeight`、`maxPixels` 内；否则接口会返回明确的客户端错误。
...
| `scale` | 否 | 固定放大倍率，只能是 `1`、`2`、`3` 或 `4`。不填时自动按 `1`、`2`、`3`、`4` 的顺序选择最小可用倍率 |
...
Modelslab Real-ESRGAN 云端超分处理器。它会先判断用户请求的 `size` 是否超过配置的最大尺寸；如果超过，会从 `2x`、`3x`、`4x` 中选择最小可用倍率，反推出发送给生成 provider 的尺寸，生成完成后把 provider 输出图片提交到 Modelslab super resolution API 放大，再下载返回的图片 URL。如果没有超过最大尺寸，则不会调用 Modelslab。

Modelslab 版本不会在云端放大后再用本地 `sharp` 做最终 resize。为了让最终尺寸由超分模型直接产出，请求的宽高必须能被所选倍率整除，并且反推后的生成尺寸要在 `maxWidth`、`maxHeight`、`maxPixels` 内；否则接口会返回明确的客户端错误。
